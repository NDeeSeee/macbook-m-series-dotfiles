...
function activate(context) {
    context.subscriptions.push(vscode_1.commands.registerCommand("rmarkdown.onEnterKey", onEnterKey), vscode_1.commands.registerCommand("rmarkdown.onCtrlEnterKey", () => {
        return onEnterKey("ctrl");
    }), vscode_1.commands.registerCommand("rmarkdown.onShiftEnterKey", () => {
        return onEnterKey("shift");
    }), vscode_1.commands.registerCommand("rmarkdown.onTabKey", onTabKey), vscode_1.commands.registerCommand("rmarkdown.onShiftTabKey", () => {
        return onTabKey("shift");
    }), vscode_1.commands.registerCommand("rmarkdown.onBackspaceKey", onBackspaceKey), vscode_1.commands.registerCommand("rmarkdown.checkTaskList", checkTaskList), vscode_1.commands.registerCommand("rmarkdown.onMoveLineDown", onMoveLineDown), vscode_1.commands.registerCommand("rmarkdown.onMoveLineUp", onMoveLineUp), vscode_1.commands.registerCommand("rmarkdown.onCopyLineDown", onCopyLineDown), vscode_1.commands.registerCommand("rmarkdown.onCopyLineUp", onCopyLineUp), vscode_1.commands.registerCommand("rmarkdown.onIndentLines", onIndentLines), vscode_1.commands.registerCommand("rmarkdown.onOutdentLines", onOutdentLines));
}
...
function onEnterKey(modifiers) {
    ...
    if (/^> /.test(textBeforeCursor)) {
        ...
        // Ordered list
        let config = vscode_1.workspace.getConfiguration("rmarkdown.orderedList").get("marker");
        let marker = "1";
...
function indent(editor) {
    ...
    if (vscode_1.workspace.getConfiguration("rmarkdown.list", editor.document.uri).get("indentationSize") === "adaptive") {
        try {
...
function fixMarker(line) {
    if (!vscode_1.workspace.getConfiguration("rmarkdown.orderedList").get("autoRenumber")) {
        return;
    }
    if (vscode_1.workspace.getConfiguration("rmarkdown.orderedList").get("marker") === "one") {
        return;
    }
    let editor = vscode_1.window.activeTextEditor;
    if (line === undefined) {
