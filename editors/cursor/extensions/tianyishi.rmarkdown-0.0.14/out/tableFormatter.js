...
function activate(_) {
    ...
    function registerFormatterIfEnabled() {
        const isEnabled = vscode_1.workspace.getConfiguration().get("rmarkdown.tableFormatter.enabled", true);
        if (isEnabled && !registration) {
            registration = vscode_1.languages.registerDocumentFormattingEditProvider(util_1.mdDocSelector, new MarkdownDocumentFormatter());
        }
    ...
    vscode_1.workspace.onDidChangeConfiguration((event) => {
        if (event.affectsConfiguration("rmarkdown.tableFormatter.enabled")) {
            registerFormatterIfEnabled();
        }
    });
}
...
class MarkdownDocumentFormatter {
     ...
     * In case of `rmarkdown.table.normalizeIndentation` is `enabled` it is rounded to the closest multiple of
     * the configured `tabSize`.
     */
    getTableIndentation(text, options) {
        let doNormalize = vscode_1.workspace.getConfiguration("rmarkdown.tableFormatter").get("normalizeIndentation");
